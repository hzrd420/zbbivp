...
class Authentication {
  ...
  protected function checkRememberCookies(): ?int {
    ...
    $f3->set('COOKIE.identifier', $identifier, time() + (3600*24*$tokenLifetime));
    $f3->set('COOKIE.securitytoken', $newSecurityToken, time() + (3600*24*$tokenLifetime));
  ...
  protected function setRememberCookies(int $userId): void {
    ...
    $f3->set('COOKIE.identifier', $identifier, time() + (3600*24*$tokenLifetime));
    $f3->set('COOKIE.securitytoken', $securityToken, time() + (3600*24*$tokenLifetime));
  } // setRememberCookies()
